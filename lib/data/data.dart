...
// This file contains the mock data for 100 diamonds
class DiamondData {
  ...
  // Method to get unique values for filter options
  static List<String> getLabOptions() {
    final labs = diamonds.map((d) => d.lab).toSet().toList();
    labs.sort();
    return labs;
  }

  static List<String> getShapeOptions() {
    final shapes = diamonds.map((d) => d.shape).toSet().toList();
    shapes.sort();
    return shapes;
  }

  static List<String> getColorOptions() {
    final colors = diamonds.map((d) => d.color).toSet().toList();
    colors.sort();
    return colors;
  }

  static List<String> getClarityOptions() {
    final clarities = diamonds.map((d) => d.clarity).toSet().toList();
    clarities.sort();
    return clarities;
  }

  static double getMinCarat() {
    return diamonds.map((d) => d.carat).reduce((a, b) => a < b ? a : b);
  }

  static double getMaxCarat() {
    return diamonds.map((d) => d.carat).reduce((a, b) => a > b ? a : b);
  }
}
